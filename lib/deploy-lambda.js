...
function wrapLambda(workingDir, awlyCliDir, options){
    ...
    return new Promise(function(resolve, reject){
        fs.writeFile(path.join(workingDir, "./lambdas/index.js"),
            ...
            require(path.resolve(awlyCliDir, 'helpers/lambda-prepend-no-gzip.js'))(options.gzip === false ? false : true) +
            fs.readFileSync(path.resolve(awlyCliDir, './helpers/lambda-prepend.js'), 'utf8') +
            fs.readFileSync(path.resolve(workingDir, './lambdas/index.js'), 'utf8') +
            fs.readFileSync(path.resolve(awlyCliDir, './helpers/lambda-append.js'), 'utf8'),
            function(err) {
                if(err) {
                    reject(err);
                    return err;

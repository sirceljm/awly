...
function getProjectConfig(customPath) {
    const configPath = customPath || "./project-config/main.config.js";
    const projectConfig = require(path.resolve(cwd, configPath));
    if (projectConfig) {
...
function checkProjectCredentials(projectConfig){
    try{
        projectConfig.credentials = require(projectConfig.credentials_path);
        return true;
    } catch(err){

...
function runSequence(projectConfig, page, options){
    ...
    compileIntoDir = path.resolve(cwd, "./.lambdas");
    ...
    utils = require("./shared/awly-cli-utils")(AWS);
    ...
    co(function *(){
        const lassoPageResult = yield runLasso(cwd, entry, options);
        ...
        const webpackStats = yield runWebpack(cwd, awlyCliDir, entry, compiledPagePath, options);
...
function runLasso(workingDir, entry, options){
    console.log("COMPILING ASSETS");
    console.log(workingDir, entry, options);
...
function runWebpack(cwd, awlyCliDir, entry, pagePath, options){
...
function uploadLambda(lambdaZipContent, projectConfig, lambdaName){
    return utils.lambda.uploadLambda(lambdaName, lambdaZipContent, projectConfig);
}

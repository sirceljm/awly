...
function serverStart(projectConfig, options){
    ...
    const utils = require(path.resolve(awlyCliDir, "./lib/shared/utils.js"))();

    require("dotenv").config({
        path: path.resolve(awlyCliDir, ".env")
    });

    const markojsModulesPath = process.env.MARKOJS_MODULES_PATH || "@awly/marko/";
    ...
    utils.project.getProjectStructure(path.resolve(cwd, "./src/pages")).then((projectStructure) => {
        ...
        lasso.lassoPage(lassoPageOptions).then(function(lassoPageResult) {
            ...
            if (inlineCSS) {
                css = utils.compile.mergeLassoFilesIntoStyleTag(lassoPageResult);
            } else {

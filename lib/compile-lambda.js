...
function runSequence(entry, page, projectConfig){
    ...
    co(function *(){
        ...
        yield preparePageTemplate(cwd, entry, lassoPageResult);
        const stats = yield runWebpack(cwd, entry);
        // console.log(stats);
        // return;
        const lambdaPath = yield wrapLambda(cwd, entry);
    ...
    return new Promise(function (resolve, reject) {
        ...
        lassoPageResult.files.forEach((file) => {
            ...
            var params = {
                ...
                s3Params: {
                    Bucket: projectConfig.lambda_edge_s3_dummy_endpoint_bucket_name,
...
function preparePageTemplate(workingDir, entry, lassoPageResult){
    console.log('preparePageTemplate');
    return new Promise(function(resolve, reject){
        var markoCompiler = require('marko/compiler')
        markoCompiler.configure({ requireTemplates: true });
        require('marko/node-require');

        let globals = {$global:{injectCSS: lassoPageResult.getHeadHtml(), injectJS:lassoPageResult.getBodyHtml()}};
        let renderAppend = require(path.join(workingDir, 'helpers/render-append.js'))(globals);
        var compiledSrc = markoCompiler.compileFile(entry);

        fs.writeFile(entry + '.js',
            fs.readFileSync(path.join(workingDir, 'helpers/render-prepend.js'), 'utf8') +
            compiledSrc +
            renderAppend,
        function(err) {
            if(err) {
...
function compileLambda(workingDir, entry){
    console.log(path.resolve(workingDir, 'lambdas'), entry, path.relative(workingDir, entry)+'.js');
    return webpack({
        stats:{
            errorDetails: true,
        },
...
function wrapLambda(workingDir){
    console.log('wrapLambda');
    return new Promise(function(resolve, reject){
        fs.writeFile(path.join(workingDir, "./lambdas/index.js"),
            // fs.readFileSync('./helpers/graphql-prepend.js', 'utf8') +
            fs.readFileSync(path.join(workingDir, './helpers/lambda-prepend.js'), 'utf8') +
            fs.readFileSync(path.join(workingDir, './lambdas/index.js'), 'utf8') +
            fs.readFileSync(path.join(workingDir, './helpers/lambda-append.js'), 'utf8'),
            function(err) {
                if(err) {
                    reject(err);
                    return err;
    	...
    	lambda.getFunction({
    	  FunctionName: 'marko_test_edge',
    		...
    		if (err) { // NO FUNCTION FOUND - CREATE NEW
    			  ...
    			  MemorySize: projectConfig.lambda_default_memory_size || 128, // needs 128 for edge
    			...
    			lambda.createFunction(params, function(err, data) {
    		  	...
    		  	var params = {
    				FunctionName: 'marko_test_edge', /* required */
    				Publish: true,
    				ZipFile: lambdaZipContent
    			};
                ...
                if (existing_distribution) {
                    ...
                    cloudfront.getDistributionConfig(params, function (err, data) {
                        if (err) {
                            ...
                            console.log("PAGe: ", pathPattern, index);
